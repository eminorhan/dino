...
def train_dino(args):
    ...
    # ============ init schedulers ... ============
    lr_schedule = dino_utils.cosine_scheduler(args.lr, args.min_lr, args.epochs, len(data_loader), warmup_epochs=args.warmup_epochs)
    wd_schedule = dino_utils.cosine_scheduler(args.weight_decay, args.weight_decay_end, args.epochs, len(data_loader))
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, data_loader, optimizer, lr_schedule, wd_schedule, momentum_schedule, epoch, fp16_scaler, args):
    ...
    for it, (images, _) in enumerate(data_loader):
        ...
        for i, param_group in enumerate(optimizer.param_groups):
            param_group["lr"] = lr_schedule[0]  # we don't use lr scheduling
    ...
    dino_utils.save_on_master(save_dict, os.path.join(args.output_dir, args.save_prefix + f'checkpoint{epoch:04}.pth'))
