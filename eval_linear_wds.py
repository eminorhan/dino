...
def train(model, linear_classifier, optimizer, loader, epoch, args):
    ...
    for (inp, target) in metric_logger.log_every(loader, args.n_train // args.batch_size_per_gpu, header):
        # move to gpu
...
def validate_network(val_loader, model, linear_classifier, args):
    ...
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Test:'
    for inp, target in metric_logger.log_every(val_loader, args.n_val // args.batch_size_per_gpu, header):
        # move to gpu
        inp = inp.cuda(non_blocking=True)
        target = target.cuda(non_blocking=True)
